...
#'
#' @noRd 
mod_address_info_server <- function(id, data, data2, start, filter_street, filter_number){
  moduleServer( id, function(input, output, session){
    ns <- session$ns
 
    # Get Information of Address
    infosReactive <- eventReactive(start, {
      req(filter_street)
      req(filter_number)
    ...
    dataAvailable <- eventReactive(start, {
      req(filter_street)
      req(filter_number)
      # Pull district
      district <- data %>%
    ...
    # Reactive Info
    infoReactive <- eventReactive(start, {
      req(filter_street)
      req(filter_number)
